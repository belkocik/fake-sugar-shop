  ...
  const { data, error } = useSWR(
    [
      ...
      `
        ...
        sugarsConnection(orderBy: createdAt_DESC, where: {title_contains: $searchValue, tags_contains_some: ["węgiel"]}, first: 6, skip: $skip) {
          edges {
            node {
              title
              tags
...
export const getServerSideProps: GetServerSideProps = async () => {
  const data = await fetcher(
    ...
    `
    query getSugars() {
      sugarsConnection(orderBy: createdAt_DESC, where: {tags_contains_some: ["węgiel"]}, first: 6, skip: 0) {
        edges {
          node {
            title
            tags
